...
class PuzzleModel:
    ...
    def _load_puzzles(self) -> List[Puzzle]:
        ...
        for puzzle_data in data["puzzles"]:
            # Create matches with is_visible always set to True
            matches = [
                Match(
                    x1=match["x1"],
                    y1=match["y1"],
                    x2=match["x2"],
                    y2=match["y2"],
                    is_visible=True,  # Always set to True regardless of JSON value
                )
                for match in puzzle_data["matches"]
            ]
            puzzle = Puzzle(
                matches=matches,
    ...
    def _save_puzzles(self):
        ...
        data = {
            "puzzles": [
                {
                    "matches": [
                        {"x1": match.x1, "y1": match.y1, "x2": match.x2, "y2": match.y2}
    ...
    def add_puzzle(self, puzzle_data: Dict):
        ...
        solution = {"removed_matches": removed_matches}
